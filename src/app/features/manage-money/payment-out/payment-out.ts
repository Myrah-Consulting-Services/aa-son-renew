...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, Party, CreateCash, CreateBank],
  templateUrl: './payment-out.html',
  styleUrl: './payment-out.scss'
})
export class PaymentOut {
  @Input() modalRef: any;
  @Input() isModal: any
  ...
  fetchInvoicesForParty(partyId: number) {
    if(this.editinvoiceId){
      this.api.get(`/invoice/allparty-wise-sales/${partyId}/2/`+this.api.getUserCompany()+`/`).subscribe({
        next: (data: any) => {
          if (data.status == 200) {
      ...
      this.api.get(`/invoice/party-wise-sales/${partyId}/`+2+`/`+this.api.getUserCompany()+`/`).subscribe({
        next: (data: any) => {
          if (data.status == 200) {
            this.invoiceList = data.data;
            this.invoiceList.forEach(element => {
  ...
  submit() {
    ...
    if(this.editinvoiceId){
      ...
      this.api.put('/money/Edit_PaymentOut_AgainstInvoice/'+this.editinvoiceId+'/', formData).subscribe({
        next: (res: any) => {
          console.log(res);
          if(res.status == 200){
            this.receiptSaved = true;
            this.toast.show('Success','Payment sent successfully','success');
            this.close();
          }else{
            this.toast.show('Error','Payment sent failed','danger');
          }
        },
        error: (error) => {
      ...
      this.api.post('/money/paymentout_against_invoice/', formData).subscribe({
        next: (res: any) => {
          console.log(res);
          if(res.status == 200){
            this.receiptSaved = true;
  ...
  downloadReceipt() {
    ...
    const docDefinition = {
      content: [
        ...
        { text: `Payment No: ${form.receipt_no}` },
  ...
  generateReceiptNo() {
    ...
    const receiptNo = `PMT-${year}${month}`;
    return receiptNo;
  }
