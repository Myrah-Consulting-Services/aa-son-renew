...
@Component({
  ...
  styleUrls: ['./purchase-register.scss']
})
export class PurchaseRegister implements OnInit {
  ...
  constructor(
    ...
    this.filterForm = this.fb.group({
      ...
      invoice_type: ['', Validators.required], // Default to purchase invoices
  ...
  getPurchaseRegister(page: number = 1) {
    this.loading = true;
    this.error = '';
  ...
  downloadInvoice(invoice: PurchaseInvoice) {
  ...
  get totalInvoices(): number {
    return this.purchaseInvoices.length;
  }

  get totalTaxableAmount(): number {
    return this.purchaseInvoices.reduce((sum, inv) => sum + inv.taxable_amt, 0);
  }

  get totalVAT(): number {
    return this.purchaseInvoices.reduce((sum, inv) => sum + inv.total_vat, 0);
  }

  get totalDiscount(): number {
    return this.purchaseInvoices.reduce((sum, inv) => sum + inv.total_discount, 0);
  }

  get totalAmount(): number {
    return this.purchaseInvoices.reduce((sum, inv) => sum + inv.final_total_amount, 0);
  }

  get paidInvoices(): number {
    return this.purchaseInvoices.filter(inv => inv.payment_status === 3).length;
  }

  get partialInvoices(): number {
    return this.purchaseInvoices.filter(inv => inv.payment_status === 2).length;
  }

  get unpaidInvoices(): number {
    return this.purchaseInvoices.filter(inv => inv.payment_status === 1).length;
  }
